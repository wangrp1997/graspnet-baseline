...
def calculate_mask_center_depth(depth_data, mask):
    """
    计算掩码中心的9个元素的平均深度值。

    Args:
        depth_data (numpy.ndarray): 深度数据数组。
        mask (numpy.ndarray): 掩码布尔数组，True 表示掩码区域。

    Returns:
        float: 掩码中心9个元素的平均深度值。如果掩码区域不足9个像素，返回整个掩码区域的平均值。
    """
    # 获取掩码区域的坐标
    mask_indices = np.argwhere(mask)
    if mask_indices.size == 0:
        return None  # 如果掩码为空，返回 None

    # 计算掩码中心点
    center_y, center_x = np.mean(mask_indices, axis=0).astype(int)

    # 提取中心点附近的3x3区域
    h, w = depth_data.shape
    y_min, y_max = max(0, center_y - 1), min(h, center_y + 2)
    x_min, x_max = max(0, center_x - 1), min(w, center_x + 2)

    # 获取3x3区域的深度值
    center_depth_values = depth_data[y_min:y_max, x_min:x_max][mask[y_min:y_max, x_min:x_max]]

    # 如果中心区域不足9个像素，返回整个掩码区域的平均值
    depth_mean = 0
    if center_depth_values.size < 9:
        depth_mean = depth_data[mask].mean()
    else:
        depth_mean = center_depth_values.mean()
